...
export class FoodService {
  ...
  url = "http://www.omdbapi.com/?apikey=ef8b3047";
  ...
  ACCESS_KEY = 'e2cc4056c58d4d73b55ccbce157683f8';
  APP_SECRET = 'ce1af7d3363b444a9561294afe839f71';
  OAUTH_VERSION = '1.0';
  OAUTH_SIGNATURE_METHOD = 'HMAC-SHA1';

  getOauthParameters(): object {
   ...
   getSignature(queryParams: object, httpMethod = 'GET') {
    const signatureBaseString = [
      httpMethod,
      encodeURIComponent(this.API_PATH),
      //encodeURIComponent(stringify(queryParams)),
      encodeURIComponent( Object.keys(queryParams).map(key => key + '=' + queryParams[key]).join('&') ),
    ].join('&');
    const signatureKey = `${this.APP_SECRET}&`;
  ...
  makeApiCall(methodParams: object, httpMethod = 'GET'): Promise<Response> {
    const queryParams = {
    ...
    return fetch(`${this.API_PATH}?${ Object.keys(queryParams).map(key => key + '=' + queryParams[key]).join('&') }`, {method: httpMethod});
  }

  async searchFood(query: string, maxResults = 8) {
    const methodParams = {
      method: 'foods.search',
